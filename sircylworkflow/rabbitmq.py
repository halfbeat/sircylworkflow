...
class RabbitMQ:
    def __init__(self, host, port, user, password, vhost, ttl=None):
        self.host = host
        self.port = port
        self.user = user
    ...
    def consume(
        self, queue_name: str, callback: Callable, auto_ack: bool = True
    ) -> Any:
        if not self.channel:
            self.connect()
    ...
    def declare_exchange(
        self, exchange_name, exchange_type: ExchangeType | str = "direct", durable=False
    ):
        if not self.channel or self.channel.is_closed:
            self.connect()
